...
export async function POST(request: Request) {
  ...
  // Preferir Meta (WhatsApp Cloud API) de la clínica; si no, Ultramsg global.
  const cfg = await getClinicWhatsappConfig(admin, context.clinicId)
  const creds = metaCredsFrom(cfg)
  let ok = false
  if (creds) {
    ok = (await sendMetaText(creds, phone, message)).ok
  } else if (isUltramsgConfigured()) {
    ok = await sendWhatsappMessage(phone, message)
  } else {
