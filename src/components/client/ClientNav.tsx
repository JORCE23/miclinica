    ...
    <>
      {routes.map((route) => (
        <Link
          ...
          className={cn(
            "flex items-center gap-2 text-sm font-medium transition-colors hover:text-[#1A1A14]",
            pathname === route.href
              ? "text-[#1A1A14] dark:text-rose-400"
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:bg-slate-950/95 dark:supports-[backdrop-filter]:bg-slate-950/60 shadow-sm">
      <div className="container mx-auto flex h-16 items-center justify-between px-4 md:px-8">
        <div className="flex items-center gap-2">
          <Sparkles className="h-6 w-6 text-[#8A929B]" />
          <div className="flex flex-col">
            <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-rose-500 to-slate-500 leading-none">
              Mi Clínica
            </span>
            <span className="text-[10px] font-bold text-[#8A929B] uppercase tracking-wider mt-0.5">
              Portal Paciente
            </span>
          </div>
        </div>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[400px]">
              <SheetHeader>
                <SheetTitle className="text-left flex items-center gap-2">
                  <Sparkles className="h-5 w-5 text-[#8A929B]" /> Mi Clínica
                </SheetTitle>
              </SheetHeader>
